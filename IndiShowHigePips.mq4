//+------------------------------------------------------------------+
//|                                             IndiShowHigePips.mq4 |
//|                                                                  |
//|                                                                  |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property description "バーをクリックすると上ヒゲのパーセント、もう一度クリックでpips、次に下ヒゲのパーセント、pipsが表示されます。もう一度クリックで消えます。"
#property strict
...
void updatePipsTextObj(int aPosX,int aPosY) {
  ...
  for(int _shift = 0; _shift < Bars; _shift++)
  {
    ...
    double _higeUpValueAbs = getHige(1, Open[_shift], High[_shift], Low[_shift], Close[_shift]);
    double _higeDownValueAbs = getHige(-1, Open[_shift], High[_shift], Low[_shift], Close[_shift]);
    string _text = "";
    string _upString = "↑:";
    string _downString = "↓:";
    //Pipsかパーセントか
    ...
    if(_oldText == "") {
      //上ヒゲ%表示に切り替え
      double _higeUpPercent = _higeUpValueAbs / _allValue * 100;
      // Print("_percent:",_percent);
      _text = _upString + DoubleToStr(_higeUpPercent,0) + "%";
    } else if(StringFind(_oldText,_upString) != -1) {
      //上ヒゲ
      if(StringFind(_oldText,"%") != -1) {
        //上ヒゲp表示に切り替え
        double _higeUpPips = _higeUpValueAbs / gUnitPerPips;
        _text = _upString + DoubleToStr(_higeUpPips*10,0) + "p"; //MT4の十字カーソルの計測値が10倍になっているため
      } else if(StringFind(_oldText,"p") != -1) {
        //下ヒゲ%表示に切り替え
        double _higeDownPercent = _higeDownValueAbs / _allValue * 100;
        _text = _downString + DoubleToStr(_higeDownPercent,0) + "%";
      }
    } else if(StringFind(_oldText,_downString) != -1) {
      //下ヒゲ
      if(StringFind(_oldText,"%") != -1) {
        //下ヒゲp表示に切り替え
        double _higeDownPips = _higeDownValueAbs / gUnitPerPips;
        _text = _downString + DoubleToStr(_higeDownPips*10,0) + "p"; //MT4の十字カーソルの計測値が10倍になっているため
      } else if(StringFind(_oldText,"p") != -1) {
        //非表示に切り替え
        _text = "";
      }
